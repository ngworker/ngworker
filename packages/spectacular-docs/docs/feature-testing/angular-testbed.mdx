...
describe('Tour of Heroes: Crisis center integration tests', () => {
  beforeEach(async () => {
    TestBed.configureTestingModule({
      providers: [
        { provide: ComponentFixtureAutoDetect, useValue: true }, // [3]
        provideRouter([
          { path: crisisCenterPath, loadChildren: () => crisisCenterRoutes }, // [2]
        ]),
      ],
    });

    rootFixture = TestBed.createComponent(TestAppComponent); // [4]
    location = TestBed.inject(Location); // [5]
    router = TestBed.inject(Router); // [5]
    crisisService = TestBed.inject(CrisisService); // [5]
    await rootFixture.ngZone?.run(() => router.navigate([crisisCenterPath])); // [6]
  });
...
2. Register the routed feature
3. Enable automatic change detection
4. Bootstrap the test root component
5. Resolve navigation and data services
6. Initialize feature navigation
...
Now that we have a working setup, we could compare that to [setting up a test for the same feature using Spectacular](./index.mdx#setting-up-an-angular-feature-integration-test-with-spectacular).
...
These are examples of boilerplate that Spectacular takes care of as demonstrated in [Testing a complete user flow with Spectacular](./index.mdx#testing-a-complete-user-flow-with-spectacular).
