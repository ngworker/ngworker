...
Spectacular does not provide an API for interacting with the DOM. We can use Angular's testing APIs such as debug elements for this. Alternatively, we [combine Spectacular with Angular Testing Library](feature-testing/angular-testing-library) or Angular component harnesses.
...
Spectacular doesn't offer an API for interacting with or inspecting the DOM. To make this easier, [combine Spectacular with Angular Testing Library](./angular-testing-library.mdx) or Angular component harnesses.
...
When comparing a feature test using Spectacular to [a feature test using Angular testbed](./angular-testbed.mdx#reviewing-a-feature-test-using-the-angular-testbed), we notice a few convenience APIs in use:
