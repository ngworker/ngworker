...
We pass our routed feature to the [`routes`](../api/interfaces/ProvideSpectacularFeatureTestingOptions.md#routes) option, in our case `crisisCenterPath`, as part of a route that wraps it in a `loadChildren` callback.
...
When we're using Angular Testing Library, we use [`render`](https://testing-library.com/docs/angular-testing-library/api#render) and [`provideSpectacularFeatureTesting`](../api/modules.md#providespectacularfeaturetesting) instead of [`createFeatureHarness`](../api/modules.md#createfeatureharness).
...
Alternatively, we use [`TestBed.inject`](https://v16.angular.io/api/core/testing/TestBed#inject) to resolve Spectacular's feature-aware services:
...
When using Angular Testing Library, you can use Angular's [`Location#path`](https://v16.angular.io/api/common/Location#path) service method to assert navigation to feature routes:
